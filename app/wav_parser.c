...
int WAV_P_CheckValid(WAVContainer_t *container)
{
...
#endif
	{
		...
		fprintf(stderr, "set it as standard PCM file: 16it 2 channels 48K little end. \n");
		container->header.magic       = WAV_RIFF;
		container->format.magic       = WAV_FMT;
		container->format.fmt_size    = LE_INT(16);
		container->format.format      = WAV_FMT_PCM;
		container->format.channels    = 2;
		container->format.sample_rate = 48000;     /* HZ   */
		container->format.bytes_p_second = 192000; /* bytes */
		container->format.blocks_align   = 4;      /* sample bytes include both channels */
		container->format.sample_length  = 16;     /* one channel sample bits */
		container->chunk.length = -1;
		return -1;
	}
