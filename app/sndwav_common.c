...
ssize_t SNDWAV_WritePcm(SNDPCMContainer_t *sndpcm, size_t wcount)
{
	...
	if (wcount < sndpcm->chunk_size) {
		...
		//memset(data + wcount * sndpcm->bits_per_frame / 8, 0, (sndpcm->chunk_size - wcount) * sndpcm->bits_per_frame / 8);
...
int SNDWAV_SetParams(SNDPCMContainer_t *sndpcm, WAVContainer_t *wav)
{
	...
	if (buffer_time > 25000) buffer_time = 25000;  /* 50 ms */
	...
	printf("chunk size is %d, chunk bytes is %d \n", (int)(sndpcm->chunk_size), (int)(sndpcm->chunk_bytes));
